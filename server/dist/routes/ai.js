...
const router = Router();
router.post('/chat', optionalAuth, [body('message').trim().notEmpty(), body('sessionId').optional()], validate, asyncHandler(async (req, res) => {
    const { message, sessionId: sid, provider = 'openai' } = req.body;
...
router.post('/generate', [body('prompt').trim().notEmpty(), body('type').isIn(['blog', 'resume', 'tutor', 'social', 'summary'])], validate, asyncHandler(async (req, res) => {
    const { prompt, type } = req.body;
    const content = await generateContent(prompt, type);
    res.json({ content });
}));
router.post('/analyze', [body('text').trim().notEmpty()], validate, asyncHandler(async (req, res) => {
